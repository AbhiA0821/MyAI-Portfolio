    ...
    <section id="projects" className="py-24 bg-[#050505] relative overflow-hidden border-b border-slate-800/80">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 space-y-12">
        ...
        <div className="text-center max-w-3xl mx-auto space-y-3">
          ...
          <RevealText
            text="FEATURED ENGINEERING CASE STUDIES"
            as="h2"
        ...
        {/* Projects Cards Grid with Parallax and 3D Tilt */}
        <ParallaxElement speed={20}>
          <motion.div layout className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            <AnimatePresence>
              {filteredProjects.map((project) => (
                <TiltCard key={project.id} className="h-full">
                  <div className="group rounded-2xl bg-[#0A0A1A] border border-slate-800/90 hover:border-cyan-500/50 backdrop-blur-xl overflow-hidden shadow-2xl shadow-black/60 flex flex-col justify-between transition-all h-full">
                    <div>
                    ...
                    {/* Card Footer Actions */}
                    <div className="px-6 py-4 border-t border-slate-800/80 bg-slate-950/40 flex items-center justify-between mt-auto">
                      <a
