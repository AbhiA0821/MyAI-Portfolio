...
2. If the user asks to explain a specific project or "explain any one", pick the top relevant project from the retrieved context and explain its key details clearly.
3. Include clear source citations at the end of your response (e.g. Source: Knowledge / Projects).
...
class SpecializedAgent:
    ...
    async def run_stream(self, query: str, conversation_history: Optional[List[Dict[str, str]]] = None) -> AsyncGenerator[str, None]:
        docs = rag_engine.search(query, category_filter=self.category_filter, history=conversation_history)
    ...
    def _synthesize_grounded_fallback(self, query: str, docs: List[Dict[str, Any]]) -> str:
        ...
        source_label = primary_doc.get("source_type", "Knowledge / Projects")
...
class MasterOrchestrator:
    ...
    def route(self, query: str, conversation_history: Optional[List[Dict[str, str]]] = None) -> SpecializedAgent:
        query_meta = rag_engine.preprocess_query(query, history=conversation_history)
    ...
    async def route_and_execute_stream(
        ...
        agent = self.route(query, conversation_history=conversation_history)
